...
export default function Page() {
  const intro = (
    <p>
      Recurrent neural networks formed the basis of many early language models. As we will see, their sequential structure makes them well-suited for formal connections to many classical sequential models of computation, resulting in a rich field of study of their representational capacity. We begin by defining recurrent neural networks and language models.
    </p>
  );
  ...
  const sections = [
    {
      ...
      content: (
        <>
          <p>
            Recurrent neural LMs are LMs whose conditional probabilities are given by a recurrent neural network. Most of the results we discuss pertain to <strong>Elman RNNs</strong> as they are the easiest to analyze and are special cases of more common networks, e.g., those based on long short-term memory and gated recurrent units. Interestingly, the more elaborate and empirically more successful LSTM network is also theoretically more powerful in some sense; we will emphasize some of these differences below.
          </p>
          {/* RNNContainer could be added here if you have a React version */}
          ...
          <p>
            where <MathJax inline>{"$\\hiddStatet \\in \\Q^\\hiddDim$"}</MathJax> is the hidden state vector at time step <MathJax inline>{"$t$"}</MathJax>, <MathJax inline>{"$\\rnnInitstate \\in \\Q^\\hiddDim$"}</MathJax> is an initialization parameter, <MathJax inline>{"$\\symt \\in \\alphabet$"}</MathJax> is the input symbol at time step <MathJax inline>{"$t$"}</MathJax>, <MathJax inline>{"$\\inEmbedding: \\alphabet \\to \\Q^\\embedDim$"}</MathJax> is a symbol representation function, <MathJax inline>{"$\\recMtx \\in \\Q^{\\hiddDim \\times \\hiddDim}$"}</MathJax> and <MathJax inline>{"$\\inMtx \\in \\Q^{\\hiddDim \\times \\embedDim}$"}</MathJax> are parameter matrices, <MathJax inline>{"$\\biasVec \\in \\Q^{\\hiddDim}$"}</MathJax> is a bias vector, and <MathJax inline>{"$\\activation: \\Q^\\hiddDim \\to \\Q^\\hiddDim$"}</MathJax> is an element-wise non-linear activation function.
          </p>
          <p>
          ...
          <p>
            The choice of the non-linear activation function <MathJax inline>{"$\\activation$"}</MathJax> is crucial for the representation capacity of the Elman RNN. Common examples include the element-wise application of <MathJax inline>{"$\\ReLU$"}</MathJax>, i.e., <MathJax inline>{"$\\ReLU(x) = \\max(0, x)$"}</MathJax>, the Heaviside function <MathJax inline>{"$\\heaviside(x) = \\ind{x > 0}$"}</MathJax>, and the sigmoid function <MathJax inline>{"$\\sigmoid(x) = \\frac{1}{1 + \\exp(-x)}$"}</MathJax>. <MathJax inline>{"$\\ReLU$"}</MathJax> is the most common choice in modern deep learning and the focus of our analysis.
          </p>
          <p>
